...
export default function FdeThankYou({ subdomain = "fde" }: { subdomain?: string }) {
  return (
    <div className="min-h-screen bg-[#f5f7fb] text-[#17243a] font-sans flex flex-col justify-between">
      ...
      <main className="w-[min(860px,92%)] mx-auto py-12 sm:py-16 my-auto">
        <div className="bg-white rounded-[28px] border border-[#e4e8ef] shadow-xl p-8 sm:p-12 text-center relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-[#059669] via-[#34d399] to-[#0284c7]" />
          ...
          <span className="inline-block px-3.5 py-1 bg-emerald-50 text-emerald-800 text-xs font-black rounded-full border border-emerald-200 uppercase tracking-wider mb-3">Enquiry Received · FDE Course</span>
          <h1 className="text-2xl sm:text-4xl font-black text-[#064e3b] tracking-tight mb-3">Thank You for Your Interest!</h1>
          <p className="text-sm sm:text-base text-[#667085] max-w-xl mx-auto leading-relaxed mb-8">
            Your enquiry for the <strong className="text-[#17243a]">Inxyme Full Stack Development (FDE) Course</strong> has been received. Our advisor will connect with you shortly to walk you through the curriculum, tech stack, and career opportunities.
          </p>
          <div className="bg-[#f8fafc] rounded-2xl p-6 sm:p-8 border border-[#e2e7ee] text-left max-w-2xl mx-auto mb-8">
            <h2 className="text-xs font-black uppercase tracking-wider text-[#059669] mb-4">What Happens Next?</h2>
            <div className="space-y-4">
              {[["Advisor Call Within 24 Hours","Our Full Stack expert will explain the complete MERN stack curriculum, weekend batch schedule, and cloud modules."],["Fee & EMI Discussion","Get personalised pricing, available scholarships, and 0% EMI plans tailored to your profile."],["Batch Registration & Dashboard Access","Complete enrollment and get immediate access to your Inxyme dashboard with starter materials and community."]].map(([title, desc], i) => (
                <div key={i} className="flex items-start gap-3.5">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#059669] to-[#0284c7] text-white flex items-center justify-center text-xs font-black shrink-0 mt-0.5">{i+1}</div>
                  <div>
                    <h3 className="text-xs sm:text-sm font-bold text-[#064e3b]">{title}</h3>
